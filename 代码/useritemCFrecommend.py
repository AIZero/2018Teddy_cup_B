...
def getUseritemDataStructure(moivelist):
    # userDict[10001]=[1,4,8,9].... 表示用户10001看过影视1，4，8，9
...
def evalate(userId,userDict,itemUser):
    train_data=[]
    test_data = []
    train_data=random.sample(userDict[userId],int(len(userDict[userId])*0.9))
    ...
    for item in train_data:
        ...
        for neighbor in neighbors:
            for item in userDict[neighbor]:
                if item not in movie and item not in train_data:
                    movie.append(item)
                    movie_number[item] = 1
                else:
    ...
    for item in commands:
        if item[1] in test_data:
            return commands,1
    return commands,0

# 使用UseritemCF进行推荐，输入：文件名,用户ID,推荐影视数量
def recommendByUseritemCF(filename, userId):
    ...
    command_dict,a= evalate(userId, userDict, itemUser)
...
if __name__ == '__main__':
    ...
    sum = 0
    for userId in users:
        command_dict, a = recommendByUseritemCF("...\data\用户.xlsx", userId)
        sum += a
    print('准确率' + str(sum / user_number))
